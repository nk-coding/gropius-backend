...
export function createStrategyInstanceInputCheck(input: CreateStrategyInstanceInput): CreateStrategyInstanceInput {
    ...
    if (typeof input.type != "string" || input.type.trim().length <= 0) {
        throw new HttpException("Type of stragety must be specified on creation", HttpStatus.BAD_REQUEST);
    }
    ...
    if (input.isSelfRegisterActive != undefined && typeof input.isSelfRegisterActive != "boolean") {
        throw new HttpException("If isSelfRegisterActive is given it must be a valid boolean", HttpStatus.BAD_REQUEST);
    }
    ...
    if (input.doesImplicitRegister != undefined && typeof input.doesImplicitRegister != "boolean") {
        throw new HttpException("If doesImplicitRegister is given it must be a valid boolean", HttpStatus.BAD_REQUEST);
    }
    return input;
}
