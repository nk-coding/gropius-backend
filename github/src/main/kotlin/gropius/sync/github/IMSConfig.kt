...
class IMSConfig(
    ...
    private val jsonVersionString = """"${'$'}schema": "https://json-schema.org/draft/2020-12/schema\""""
    private val commonTemplateFields =
        mapOf("bot-user" to """{$jsonVersionString, "type": ["null", "string"]}""")
    private val imsTemplateName = "Github"
    private val imsTemplateFields =
        mapOf("read-user" to """{$jsonVersionString, "type": "string", "gropius-node": "IMSUser"}""") + commonTemplateFields
    private val imsProjectTemplateName = "Github"
    private val imsProjectTemplateFields =
        mapOf("read-user" to """{$jsonVersionString, "type": ["null", {"type": "string", "gropius-node": "IMSUser"}]}""") + commonTemplateFields
